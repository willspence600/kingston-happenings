...
export default function VenueSelector({
  ...
  // Check if user wants to create new venue
  ...
  // Reset search when a venue is selected
  useEffect(() => {
  ...
  const handleInputChange = (value: string) => {
    ...
    // If user types and no exact match, show option to create new venue
    if (value.trim() !== '' && !filteredVenues.some(v => v.name.toLowerCase() === value.toLowerCase().trim())) {
      setShowNewVenueFields(false); // Don't auto-show fields, wait for user to click "Create new venue"
    }
  };
  ...
  const handleCreateNewVenue = () => {
    ...
    setIsOpen(false);
    // Set the venue name from search query
          ...
          <div className="absolute z-50 w-full mt-1 bg-background border border-border rounded-xl shadow-lg max-h-64 overflow-y-auto">
            {filteredVenues.length > 0 ? (
              <>
                ...
                {/* Show option to create new venue if search doesn't match exactly */}
                {searchQuery.trim() !== '' && 
                 !filteredVenues.some(v => v.name.toLowerCase() === searchQuery.toLowerCase().trim()) && (
